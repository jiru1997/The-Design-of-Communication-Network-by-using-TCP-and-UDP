...
int main()
{
    ...
    data = "end";
    write(clientfd, data.c_str(), strlen(data.c_str()));
    printf("Completed sending applicaiont for <Student 5> \n");
    ...
    if(buffer[0] == '0')
         {printf("<Student5> has reveived the application result, the result is : rejection \n");}
    else
        {
            soc = socket(AF_INET, SOCK_DGRAM, 0);
            bzero(&udpserv_addr, sizeof(udpserv_addr));
            udpserv_addr.sin_family = AF_INET;
            udpserv_addr.sin_port = htons(UDPS5);
            inet_pton(AF_INET, "127.0.0.6", &udpserv_addr.sin_addr.s_addr);
            bind(soc, (struct sockaddr *)&udpserv_addr, sizeof(udpserv_addr));

            getsockname(soc, (struct sockaddr*)&clientAddr, &clientAddrLen);
            printf("<Student 5> has UDP port %d and IP address %s for phase 2 \n",UDPS5, inet_ntop(AF_INET, &clientAddr.sin_addr, ipAddress, sizeof(ipAddress)));
           
            printf("<Student 5> has received the application result \n");

            clieaddr_len = sizeof(udpclientAddr);
            n = recvfrom(soc, buffer, BUFSIZ, 0, (struct sockaddr *)&udpclientAddr, &clieaddr_len);
            printf("<Student 5> has reveived the application result, the result is : %s \n", buffer);
