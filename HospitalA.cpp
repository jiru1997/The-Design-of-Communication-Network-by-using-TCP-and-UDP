...
int main()
{
    ...
    string data;
    printf("<Hospital A> has sent <department> to the health center \n");
    ...
    udpserv_addr.sin_port = htons(UDPHA);
    inet_pton(AF_INET, "127.1.0.0", &udpserv_addr.sin_addr.s_addr);

    bind(sockfd, (struct sockaddr *)&udpserv_addr, sizeof(udpserv_addr));

    getsockname(sockfd, (struct sockaddr*)&clientAddr, &clientAddrLen);
    printf("<Hosptial A> has UDP port %d and IP address %s for phase 2 \n",UDPHA, inet_ntop(AF_INET, &clientAddr.sin_addr, ipAddress, sizeof(ipAddress)));
    ...
    for(int i = 0 ; i < 33; i ++)
    {
        ...
        memset(&serv_addr, 0 , sizeof(serv_addr));
        printf("<%s> has been admitted to <Hosptial A> \n",studentname.c_str());
        sleep(0.1);
    }
    
    printf("End of Phase2 for <Hosptial A> \n");
    close(sockfd);
