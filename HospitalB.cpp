...
int main()
{
    ...
    printf("<Hospital B> has TCP port %d and IP address %s for Phase 1 \n",ntohs(clientAddr.sin_port), inet_ntop(AF_INET, &clientAddr.sin_addr, ipAddress, sizeof(ipAddress)));
    printf("<Hospital B> is now connected to the health center \n");
    sleep(1);

    //------------------------------------send data------------------------------------------------

    string data;
    printf("<Hospital B> has sent <department> to the health center \n");
    ...
    udpserv_addr.sin_port = htons(UDPHB);
    inet_pton(AF_INET, "127.2.0.0", &udpserv_addr.sin_addr.s_addr);

    bind(sockfd, (struct sockaddr *)&udpserv_addr, sizeof(udpserv_addr));

    getsockname(sockfd, (struct sockaddr*)&clientAddr, &clientAddrLen);
    printf("<Hosptial B> has UDP port %d and IP address %s for phase 2 \n",UDPHB, inet_ntop(AF_INET, &clientAddr.sin_addr, ipAddress, sizeof(ipAddress)));
    ...
    for(int i = 0 ; i < 33; i ++)
    {
        ...
        memset(&serv_addr, 0 , sizeof(serv_addr));
        printf("<%s> has been admitted to <Hosptial B> \n",studentname.c_str());
        sleep(0.1);
    }
    
    printf("End of Phase2 for <Hosptial B> \n");
    close(sockfd);
